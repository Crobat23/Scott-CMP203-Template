...
namespace Skateboard
{
		...
		m_ApplicationResizing(false)
	{
		...
		// Init AssetManager
		m_AssetManager = std::make_unique<D3D::D3DAssetManager>();
	}
	...
	void WindowsPlatform::Init(const PlatformDescription& desc)
	{
		SKTBD_MSG_TRACE("Initialising the Windows Platform..");

		PlatformDescription PlatformDesc = desc;

		if (!(desc.Flags & PlatformDescriptionFlags_WindowLess))
		{
			// Create and initialise the main application window
			SKTBD_MSG_ASSERT(InitWindowsApp(PlatformDesc), "Could not successfully initialise a Windows application!")
		}
		else
		{
			if (!(desc.GraphicsContextInitDesc.Flags & GraphicsContextInitFlags_Headless))
			{
				SKTBD_MSG_ERROR("Requested Context is not headless and thus is not compatible with a windowless Skateboard, Decapitating..")
				PlatformDesc.GraphicsContextInitDesc.Flags |= GraphicsContextInitFlags_Headless;
			}
		}

		if(PlatformDesc.GraphicsContextInitDesc.Flags & GraphicsContextInitFlags_::GraphicsConetxtIntiFlags_InitWithPlatformResolution)
		{
			PlatformDesc.GraphicsContextInitDesc.BackBufferResolution = PlatformDesc.WindowSize;
		}

		// Create the API context (this could be changed to Vulkan)
		m_GraphicsContext = std::make_unique<D3D::D3DGraphicsContext>(m_MainWindow);
	...
	void WindowsPlatform::ResizeBackBuffers(WPARAM wParam, LPARAM lParam)
	{
		...
		else if (wParam == SIZE_MAXIMIZED)
		{
			...
			m_WindowClientSize = { LOWORD(lParam), HIWORD(lParam) };
		}
	...
	void WindowsPlatform::InitImGui()
	{
		...
		ID3D12DescriptorHeap* srvHeap = D3D::gD3DContext->GetSRVDescriptorHeap().GetHeap();
		D3D::D3DDescriptorHandle imguiHandle = D3D::gD3DContext->GetImGuiDescriptorHandle();

		// TODO: This is still platform dependant!
		ImGui_ImplWin32_Init(platform.GetWindow());
		ImGui_ImplDX12_Init(
			D3D::gD3DContext->GetDevice(),
			GRAPHICS_SETTINGS_NUMFRAMERESOURCES,
			D3D::gD3DContext->GetDXGIBackBufferFormat(),
			srvHeap,
	...
	{
		ID3D12GraphicsCommandList* pCommandList = D3D::gD3DContext->GetD3DDefaultFrameCommandList();
	...
	BOOL WindowsPlatform::InitWindowsApp(PlatformDescription& props)
	{
		...
		if (props.Flags & PlatformDescriptionFlags_WindowSizeAuto)
		{
			SKTBD_MSG_INFO("Sizing monitor to full size");
			...
			props.WindowSize.x = windowSize.right - windowSize.left;
			props.WindowSize.y = windowSize.bottom - windowSize.top;

			AdjustWindowRect(&windowSize, WS_OVERLAPPEDWINDOW, false);
		}
		else
		{
			windowSize = { 0, 0, static_cast<long>(props.WindowSize.x), static_cast<long>(props.WindowSize.y) };
			AdjustWindowRect(&windowSize, WS_OVERLAPPEDWINDOW, false);
		}
	...
	void WindowsPlatform::GoFullScreen()
	{
		...
		else
		{
			...
			HMONITOR NearsetMonitor =  MonitorFromWindow(m_MainWindow, MONITOR_DEFAULTTONEAREST);

			// 2. Create a MONITORINFO structure and set its size.
			MONITORINFO mi;
			mi.cbSize = sizeof(MONITORINFO);

			// 3. Call GetMonitorInfo to fill the structure.
			if (GetMonitorInfo(NearsetMonitor, &mi))
			{
				// 4. Access the RECTs.
				fullscreenWindowRect = mi.rcMonitor;
				//fullscreenWindowRect = mi.rcWork;
			}
			else
			{
	...
	LRESULT WindowsPlatform::WindowProcedure(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		//	// resize when the user is finished
			singleton.m_ApplicationIsInSizingModalLoop = true;
		...
		case WM_SIZE:
			...
			{
				if (singleton.m_ApplicationResizing & !singleton.m_ApplicationIsInSizingModalLoop)
				{
					auto e = WindowResizeEvent(singleton.m_WindowClientSize.x, singleton.m_WindowClientSize.y, singleton.m_FullscreenMode);
					PlatformDispatchEvent(e);
		...
		case WM_EXITSIZEMOVE:
			...
			singleton.m_ApplicationIsInSizingModalLoop = false;

			{
				if (singleton.m_ApplicationResizing)
				{
					auto e = WindowResizeEvent(singleton.m_WindowClientSize.x, singleton.m_WindowClientSize.y, singleton.m_FullscreenMode);
					PlatformDispatchEvent(e);
...
//				return 0;
	...
	std::unique_ptr<TimeManager> WindowsPlatform::CreateTimeManager_()
	{
		return std::make_unique<WindowsTimeManager>();
	}
}
